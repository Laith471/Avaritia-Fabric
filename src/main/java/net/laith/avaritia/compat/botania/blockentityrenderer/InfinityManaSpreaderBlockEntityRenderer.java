...
public class InfinityManaSpreaderBlockEntityRenderer implements BlockEntityRenderer<InfinityManaSpreaderBlockEntity> {

    public InfinityManaSpreaderBlockEntityRenderer(BlockEntityRendererProvider.Context ctx) {}

    @Override
    public void render(@NotNull InfinityManaSpreaderBlockEntity spreader, float partialTicks, PoseStack ms, MultiBufferSource buffers, int light, int overlay) {
        ms.pushPose();
        ...
        float r = 1, g = 1, b = 1;
        if (spreader.getVariant() == InfinityManaSpreaderBlock.Variant.INFINITY) {
    ...
    private BakedModel getCoreModel(InfinityManaSpreaderBlockEntity tile) {
        return switch (tile.getVariant()) {
            case INFINITY -> MiscellaneousModels.INSTANCE.gaiaSpreaderCore;
        };
    }
    ...
    private BakedModel getScaffoldingModel(InfinityManaSpreaderBlockEntity tile) {
        return switch (tile.getVariant()) {
            case INFINITY -> MiscellaneousModels.INSTANCE.gaiaSpreaderScaffolding;
        };
    }
}
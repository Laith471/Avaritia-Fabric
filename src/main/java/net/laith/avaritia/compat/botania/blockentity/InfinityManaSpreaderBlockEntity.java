...
public class InfinityManaSpreaderBlockEntity extends ExposedSimpleInventoryBlockEntity implements WandBindable, KeyLocked, ThrottledPacket, ManaSpreader, Wandable {
    ...
    public InfinityManaSpreaderBlockEntity(BlockPos pos, BlockState state) {
        super(Botania.INFINITY_MANA_SPREADER_BLOCK_ENTITY, pos, state);
    }
    ...
    public static void commonTick(Level level, BlockPos worldPosition, BlockState state, InfinityManaSpreaderBlockEntity self) {
        ...
        for (Direction dir : Direction.values()) {
            ...
            if (level.hasChunkAt(relPos)) {
                ...
                if (receiverAt instanceof ManaPool pool) {
                    if (wasInNetwork && (pool != self.receiver)) {
        ...
        if (control != null) {
                control.onControlledSpreaderTick(lens, self, false);
            shouldShoot = shouldShoot && control.allowBurstShooting(lens, self, false);
        }
    ...
    private void tryShootBurst() {
        if ((receiver != null) && !invalidTentativeBurst) {
            if (canShootBurst && (receiver.canReceiveManaFromBursts() && !receiver.isFull())) {
    ...
    public InfinityManaSpreaderBlock.Variant getVariant() {
        Block b = getBlockState().getBlock();
        if (b instanceof InfinityManaSpreaderBlock spreader) {
            return spreader.variant;
        } else {
            return InfinityManaSpreaderBlock.Variant.INFINITY;
        }
    }
    ...
    private ManaBurstEntity getBurst(boolean fake) {
        InfinityManaSpreaderBlock.Variant variant = getVariant();
        float gravity = 0F;
    ...
    public static class WandHud implements WandHUD {
        private final InfinityManaSpreaderBlockEntity spreader;

        public WandHud(InfinityManaSpreaderBlockEntity spreader) {

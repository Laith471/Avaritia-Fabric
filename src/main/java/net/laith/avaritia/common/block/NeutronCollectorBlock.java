...
public class NeutronCollectorBlock extends BlockWithEntity {
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (blockEntity instanceof NeutronCollectorBlockEntity) {
                ItemScatterer.spawn(world, pos, (NeutronCollectorBlockEntity)blockEntity);
                // update comparators
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, ModBlockEntities.NEUTRON_COLLECTOR_BLOCK_ENTITY, NeutronCollectorBlockEntity::tick);
    }
}
